...
const _decorateGraphData = (orders) => {
    // Group orders by second for the graph
    orders = groupBy(orders, (o) => moment.unix(o.timestamp).startOf('second').format())

    const seconds = Object.keys(orders)

    // Build the graph series
    const graphData = seconds.map((second) => {
        // Fetch all orders from the current second
        const group = orders[second]
